...
面向维护者：将 `github.com/12Jack21/remnaplus-alpine-node` 发布到 GitHub Releases，供 `install-node.sh` / `upgrade.sh` 一键安装。

## 前置条件

- GitHub 仓库：`12Jack21/remnaplus-alpine-node`
...
```bash
curl -fsSL https://raw.githubusercontent.com/12Jack21/remnaplus-alpine-node/v1.0.0/scripts/upgrade.sh | sudo bash -s -- --yes
